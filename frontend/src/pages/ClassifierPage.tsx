...
function TreeCategoryNode({
  node, selectedId, onSelect, onAdd, onRename, onDelete, onMove, onNavigate,
}: {
  node: TreeAnyNode; selectedId: number | null;
  ...
  onMove: (id: number, name: string) => void; onNavigate: (posId: number) => void;
}) {
      ...
      >
        ...
        <div className="tree-actions">
          ...
          <button className="btn btn-ghost btn-icon btn-xs" title="Удалить"
            style={{ color: '#ef4444' }}
            onClick={e => { e.stopPropagation(); onDelete(node.id, node.name); }}>✕</button>
        </div>
      </div>
      {open && hasChildren && node.children.map(child => (
        <div key={`${child.type}-${child.id}`} style={{ marginLeft: 16 }}>
          <TreeCategoryNode node={child} selectedId={selectedId}
            onSelect={onSelect} onAdd={onAdd} onRename={onRename}
            onDelete={onDelete} onMove={onMove} onNavigate={onNavigate} />
        </div>
      ))}
    </div>
  );
...
export default function ClassifierPage() {
  ...
  const invalidateCats   = () => { qc.invalidateQueries({ queryKey: ['categories'] }); qc.invalidateQueries({ queryKey: ['categories-tree'] }); };
  ...
  const breadcrumb = selectedParents.length > 0
    ? [...selectedParents].reverse().map(p => p.name).join(' / ') + (selectedCat ? ' / ' + selectedCat.name : '')
    : selectedCat?.name ?? '';
  ...
  return (
    <>
              ...
              <>
                <div className="card-header">
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                    <span>Параметры: <strong>{selectedCatDetail?.name ?? selectedCat.name}</strong>
                      <span className="badge badge-gray" style={{ marginLeft: 6, fontSize: 10 }}>ID {selectedCat.id}</span>
                    </span>
                    {breadcrumb && (
                ...
                <div className="card-body" style={{ flex: 1, overflowY: 'auto', padding: 0 }}>
                  ...
                  {catParams.length > 0 && (
                    <div className="table-wrap">
                      <table>
                        <thead>
                          <tr><th>#</th><th>Параметр</th><th>Порядок</th><th>Мин</th><th>Макс</th><th></th></tr>
                        </thead>
                        <tbody>
                          {[...catParams].sort((a, b) => a.order_num - b.order_num).map(cp => (
                            <tr key={cp.parameter_id}>
                              ...
                              <td>
                                <button className="btn btn-ghost btn-icon btn-xs" style={{ color: '#ef4444' }}
                                  onClick={() => removeParamMut.mutate({ catId: selectedId!, paramId: cp.parameter_id })}>✕</button>
                              </td>
                            </tr>
                          ))}
                        </tbody>
      ...
      {/* ── Modals ── */}
      <Modal open={addModal.open} onClose={() => setAddModal({ open: false, parentId: null })}
        ...
        footer={
          <>
            ...
            <button className="btn btn-primary" disabled={!newCatName.trim() || createMut.isPending}
              onClick={() => createMut.mutate({ name: newCatName.trim(), parent_id: addModal.parentId })}>Создать</button>
          </>
        }>
        {addModal.parentId && (
          <div className="form-hint">Родитель: <strong>{cats.find(c => c.id === addModal.parentId)?.name}</strong></div>
        )}
        <div className="form-group">
          <label className="form-label">Название</label>
          <input className="form-control" placeholder="Введите название класса" value={newCatName}
            onChange={e => setNewCatName(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && newCatName.trim() && createMut.mutate({ name: newCatName.trim(), parent_id: addModal.parentId })}
            autoFocus />
        </div>
      </Modal>
        ...
        footer={
          <>
            ...
            <button className="btn btn-primary" disabled={!renameVal.trim() || renameMut.isPending}
              onClick={() => renameModal && renameMut.mutate({ id: renameModal.id, name: renameVal.trim() })}>Сохранить</button>
          </>
        }>
        <div className="form-group">
        ...
        footer={
          <>
            ...
            <button className="btn btn-danger" disabled={deleteMut.isPending}
              onClick={() => deleteModal && deleteMut.mutate({ id: deleteModal.id, cascade })}>Удалить</button>
          </>
        }>
        ...
        footer={
          <>
            ...
            <button className="btn btn-primary" disabled={moveMut.isPending}
              onClick={() => moveModal && moveMut.mutate({ id: moveModal.id, parentId: moveTarget === '' ? null : Number(moveTarget) })}>Переместить</button>
          </>
        }>
      ...
      <Modal open={addParamModal} onClose={() => setAddParamModal(false)}
