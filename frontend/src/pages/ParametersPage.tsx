...
export default function ParametersPage() {
  ...
  const createMut = useMutation({
    mutationFn: () => parametersApi.create({
  ...
  return (
    <>
      ...
      {/* Add modal */}
      <Modal open={addModal} onClose={() => setAddModal(false)} title="Новый параметр"
        footer={
          <>
            <button className="btn btn-secondary" onClick={() => setAddModal(false)}>Отмена</button>
            <button className="btn btn-primary"
              disabled={!form.short_name.trim() || !form.name.trim() || createMut.isPending || (isEnum && !form.enum_type_id)}
        ...
        <div className="form-row">
          <div className="form-group" style={{ flex: '0 0 140px' }}>
            ...
            <input className="form-control font-mono" value={form.short_name}
              onChange={e => setForm(f => ({ ...f, short_name: e.target.value }))}
              placeholder="calories" autoFocus />
          </div>
        ...
        <div className="form-group">
          ...
          <select className="form-control" value={form.param_type_code}
            onChange={e => setForm(f => ({ ...f, param_type_code: e.target.value, enum_type_id: '' }))}>
            {PARAM_TYPE_CODES.map(t => <option key={t.code} value={t.code}>{t.label} ({t.code})</option>)}
          </select>
        </div>
        {isEnum && (
          <div className="form-group">
            <label className="form-label">Тип перечисления</label>
            <select className="form-control" value={form.enum_type_id}
              onChange={e => setForm(f => ({ ...f, enum_type_id: e.target.value }))}>
              <option value="">— Выберите —</option>
              {enumTypes.map(et => <option key={et.id} value={et.id}>{et.name} ({et.code})</option>)}
            </select>
            {enumTypes.length === 0 && <span className="form-hint text-danger">Нет перечислений. Создайте в разделе «Перечисления».</span>}
          </div>
        )}
        {(form.param_type_code === 'real' || form.param_type_code === 'integer') && (
          <div className="form-group">
            <label className="form-label">Единица измерения</label>
            <select className="form-control" value={form.unit_id}
      ...
      {/* Delete modal */}
      <Modal open={deleteModal !== null} onClose={() => setDeleteModal(null)} title="Удалить параметр"
        footer={
          <>
        ...
        <p>Удалить параметр <strong>«{deleteModal?.name}»</strong>?</p>
        <p className="text-muted" style={{ marginTop: 6, fontSize: 12 }}>Параметр будет удалён из всех классов, где он используется.</p>
      </Modal>
    </>
  );
}
