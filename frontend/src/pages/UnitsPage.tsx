...
export default function UnitsPage() {
	...
	const createMut = useMutation({
		mutationFn: () =>
			unitsApi.create({ code: form.code.trim(), name: form.name.trim(), symbol: form.symbol.trim() }),
		onSuccess: () => {
			invalidate();
			setAddModal(false);
	...
	const updateMut = useMutation({
		mutationFn: ({ id }: { id: number }) =>
			unitsApi.update(id, { code: form.code.trim(), name: form.name.trim(), symbol: form.symbol.trim() }),
		onSuccess: () => {
			invalidate();
			setEditModal(null);
	...
	return (
		<>
								...
								<tbody>
									{units.map((u) => (
										<tr key={u.id}>
											...
											<td>
												<span className="font-mono badge badge-gray">{u.code}</span>
											</td>
				...
				footer={
					<>
						...
						<button
							...
							onClick={() => editModal && updateMut.mutate({ id: editModal.id })}
						>
							Сохранить
						</button>
					</>
			...
			>
				<p>
					Удалить единицу <strong>«{deleteModal?.name}»</strong> ({deleteModal?.symbol})?
				</p>
			</Modal>
		</>
	);
